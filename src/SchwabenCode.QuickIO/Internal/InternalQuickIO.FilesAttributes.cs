...
namespace SchwabenCode.QuickIO.Internal
{
    internal static partial class InternalQuickIO
    {
        ...
        /// <param name="updatedAttributes">Attributes after add</param>
        /// <returns>true if added. false if already exists in attributes</returns>
        public static bool TryAddAttribute( string path, FileAttributes attribute , out FileAttributes updatedAttributes)
        {
            Contract.Requires( !String.IsNullOrWhiteSpace( path ) );

            // read uncached attributes
            updatedAttributes = GetAttributes( path );

            if( /* contains */ (updatedAttributes & attribute ) != attribute )
            {
                // is not present but will be set now

                updatedAttributes |= attribute;
                SetAttributes( path, updatedAttributes);
        ...
        /// <param name="updatedAttributes">Attributes after remove</param>
        /// <returns>true if removed. false if not exists in attributes</returns>
        public static bool TryRemoveAttribute( String path, FileAttributes attribute, out FileAttributes updatedAttributes )
        {
            Contract.Requires( !String.IsNullOrWhiteSpace( path ) );
