    ...
    public partial class QuickIOTransferFileCopyService : QuickIOTransferServiceBase
    {
        ...
        public void Start()
        {
            try
            {
                ...
                string targetUnc = QuickIOPath.ToPathUnc( TargetFullName );

                List<QuickIOTransferJob> fileTransferQueueItems = new List<QuickIOTransferJob>( );

                for (int index = 0; index < SourceFileInfos.Count; index++)
                {
                    QuickIOFileInfo file = SourceFileInfos[index];

                    var target = targetUnc + QuickIOPath.DirectorySeparatorChar + file.Name;
                    fileTransferQueueItems.Add(new QuickIOTransferFileCopyJob(file.FullNameUnc, target, MaxBufferSize,
                        Overwrite, true));
