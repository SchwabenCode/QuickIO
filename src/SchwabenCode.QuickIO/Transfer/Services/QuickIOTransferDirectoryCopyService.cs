    ...
    public partial class QuickIOTransferDirectoryCopyService : QuickIOTransferServiceBase
    {
        ...
        public void Start()
        {
            try
            {
                ...
                // Load prefences
                var preferences = DetermineDirectoryTransferPrefences(SourceDirectoryInfo.FullNameUnc, TargetFullName, SearchOption, Overwrite);
        ...
        private InternalDirectoryTransferPrefences DetermineDirectoryTransferPrefences(string path, String targetFullName, SearchOption searchOption, Boolean overwrite, QuickIOEnumerateOptions enumerateOptions = QuickIOEnumerateOptions.None)
        {
            var prefences = new InternalDirectoryTransferPrefences();

            var allContentUncPaths = InternalEnumerateFileSystem.EnumerateWin32FileSystemEntries(path, QuickIOPatternConstants.All, searchOption, enumerateOptions);
            ...
            {
                var currentPath = entry.Item1;
                var currentEntry = entry.Item2;

                var target = targetPathInfo.FullNameUnc + currentPath.Substring(path.Length);

                switch(currentEntry.FileSystemEntryType)
                {
                    ...
                    case QuickIOFileSystemEntryType.File:
                        {
                            prefences.FileTransferQueueItems.Add(new QuickIOTransferFileCopyJob(currentPath, target, MaxBufferSize, overwrite, false));
                        }
                        break;
                }
            }
