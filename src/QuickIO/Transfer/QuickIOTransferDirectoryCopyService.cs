...
public partial class QuickIOTransferDirectoryCopyService : QuickIOTransferServiceBase
{
    ...
    /// </code>
    /// </example>
    public QuickIOTransferDirectoryCopyService(QuickIODirectoryInfo sourceDirectoryInfo, string targetFullName, int threadCount = 1, int retryCount = 3, SearchOption searchOption = SearchOption.TopDirectoryOnly, bool overwrite = false)
    ...
    /// </code>
    /// </example>
    public QuickIOTransferDirectoryCopyService(IQuickIOTransferObserver? observer, QuickIODirectoryInfo sourceDirectoryInfo, string targetFullName, int threadCount = 1, int retryCount = 3, SearchOption searchOption = SearchOption.TopDirectoryOnly, bool overwrite = false)
    ...
    public void Start()
    {
        try
        {
            ...
            // Load prefences
            InternalDirectoryTransferPrefences preferences = DetermineDirectoryTransferPrefences( SourceDirectoryInfo, TargetFullName, SearchOption, Overwrite );
    ...
    private InternalDirectoryTransferPrefences DetermineDirectoryTransferPrefences(QuickIODirectoryInfo sourceDirectoryInfo, string targetFullName, SearchOption searchOption, bool overwrite)
    {
        InternalDirectoryTransferPrefences prefences = new( );
        ...
        QuickIOPathInfo targetPathInfo = new( targetFullName );

        prefences.CreateDirectoryJobs = [];
        prefences.FileTransferQueueItems = [];
        foreach (KeyValuePair<string, QuickIOFileSystemEntryType> entry in allContentUncPaths)
        {
            string target = targetPathInfo.FullNameUnc + entry.Key.Substring( sourceDirectoryInfo.FullNameUnc.Length );
