...
, System.Threading.CancellationToken cancellationToken = default( System.Threading.CancellationToken )
            ...
            base( observer, prorityLevel, overwrite
, cancellationToken
 )
        {
            if( String.IsNullOrEmpty( source ) )
            {
                throw new ArgumentException( "Parameter cannot be empty.", nameof( source ) );
            }
            if( String.IsNullOrEmpty( target ) )
            {
                throw new ArgumentException( "Parameter cannot be empty.", nameof( target ) );
            }
