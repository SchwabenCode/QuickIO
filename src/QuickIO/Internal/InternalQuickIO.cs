...
internal static class InternalQuickIO
{
    ...
    public static void CreateDirectory(QuickIOPathInfo pathInfo, bool recursive = false)
    {
        ...
        if (pathInfo.IsRoot)
        {
            throw new PathNotFoundException("Root directory " + pathInfo.FullName + "  does not exists and cannot be created.");
        }

        if (recursive)
        {
            QuickIOPathInfo? parent = pathInfo.Parent ?? throw new PathNotFoundException("Root directory " + pathInfo.FullName + "  does not exists and cannot be created.");
            if (parent.IsRoot)
            {
                // Root
                if (!parent.Exists)
    ...
    internal static QuickIODirectoryMetadata? EnumerateDirectoryMetadata(
        string uncDirectoryPath, Win32FindData findData, QuickIOEnumerateOptions enumerateOptions)
    {
        // Results
    ...
    private static IEnumerable<KeyValuePair<QuickIOPathInfo, QuickIOFileSystemEntryType>> EnumerateFileSystemEntries(string uncDirectoryPath, string pattern = QuickIOPatternConstants.All, SearchOption searchOption = SearchOption.TopDirectoryOnly, QuickIOEnumerateOptions enumerateOptions = QuickIOEnumerateOptions.None)
    {
        ...
        do
        {
            ...
            if (InternalHelpers.ContainsFileAttribute(win32FindData.dwFileAttributes, FileAttributes.Directory))
            {
                ...
                if (searchOption == SearchOption.AllDirectories)
                {
                    foreach (KeyValuePair<QuickIOPathInfo, QuickIOFileSystemEntryType> match in EnumerateFileSystemEntries(new QuickIOPathInfo(resultPath, win32FindData.cFileName), pattern, searchOption, enumerateOptions))
                    {
                        yield return match;
                    }
                }
    ...
    {
        /* PInvoke request */
        if (!Win32SafeNativeMethods.GetDiskFreeSpaceEx(rootPath, out ulong freeBytes, out ulong totalBytes, out ulong totalFreeBytes))
        {
            int win32Error = Marshal.GetLastWin32Error( );
        ...
        string path, QuickIOEnumerateOptions enumerateOptions = QuickIOEnumerateOptions.None)
    {
        ...
        using (Win32FileHandle fileHandle = FindFirstSafeFileHandle(currentPath, win32FindData, out int win32Error))
        {
            ...
            do
            {
                ...
                {
                    fileCount++;
                    totalSize += win32FindData.CalculateBytes(); _ = win32FindData.CalculateBytes();
                }
                else
                {
                    folderCount++;
        ...
        // Return result;
        return new QuickIOFolderStatisticResult(0, fileCount, 0);
    }
