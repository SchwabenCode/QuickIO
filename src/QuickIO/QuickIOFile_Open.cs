...
public static partial class QuickIOFile
{
    ...
    /// Opens a <see cref="FileStream"/> 
    /// </summary>
    private static FileStream OpenFileStream(QuickIOPathInfo pathInfo, FileAccess fileAccess, FileMode fileOption = FileMode.Open, FileShare shareMode = FileShare.Read, int buffer = 0)
    {
        SafeFileHandle fileHandle = Win32SafeNativeMethods.CreateFile( pathInfo.FullNameUnc, fileAccess, shareMode, IntPtr.Zero, fileOption, 0, IntPtr.Zero );
    ...
    /// Opens a <see cref="FileStream"/> 
    /// </summary>
    private static FileStream OpenAppendFileStream(QuickIOPathInfo pathInfo, FileAccess fileAccess, FileMode fileOption = FileMode.Open, FileShare shareMode = FileShare.Read, int buffer = 0)
    {
        Microsoft.Win32.SafeHandles.SafeFileHandle fileHandle = Win32SafeNativeMethods.CreateFileForAppend( pathInfo.FullNameUnc, 0x0004 /* Internal Win Append Mode*/, shareMode, IntPtr.Zero, fileOption, 0, IntPtr.Zero );
        int win32Error = Marshal.GetLastWin32Error( );
        if (fileHandle.IsInvalid)
        {
            InternalQuickIOCommon.NativeExceptionMapping(pathInfo.FullName, win32Error); // Throws an exception
